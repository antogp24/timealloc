...
main :: proc() {
    initial_screen_w: i32 = 12*HOUR_BLOCK_W + 12*BLOCK_PADDING + WINDOW_PADDING_X
    initial_screen_h: i32 = 2*HOUR_BLOCK_H + WINDOW_PADDING_Y
    ...
    for !rl.WindowShouldClose() {
        ...
        for hour: f32; hour <= 24; hour += 1 {
            ...
            if (hour < 12) { 
                rect_pos.x = screen_w/2 + HOUR_BLOCK_W*(hour - 6) + BLOCK_PADDING*(hour - 6)
                rect_pos.y = screen_h/2 - HOUR_BLOCK_H
            } else {
                rect_pos.x = screen_w/2 + HOUR_BLOCK_W*(hour - 18) + BLOCK_PADDING*(hour - 18)
                rect_pos.y = screen_h/2 + number_h
            }
            
            rect := rl.Rectangle{rect_pos.x, rect_pos.y, HOUR_BLOCK_W, HOUR_BLOCK_H}
            h, s, v: f32
            x := hour / 24
            ...
            if (hour == 12) {
                text_pos := rl.Vector2{
                    screen_w/2 + HOUR_BLOCK_W*6 + BLOCK_PADDING*6,
                    screen_h/2 - HOUR_BLOCK_H - number_h/2,
                }            
                draw_text_centered(hour_text, text_pos)
            }
            ...
            for i in 0..<4 {
                if hour == 24 do break
                if i == 0 do continue
                start := rect_pos + rl.Vector2{cast(f32)i*HOUR_BLOCK_W*0.25, 0}
